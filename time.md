...
Изменяем данные, привязанные к GPG-ключу

Важно: это необязательный шаг. Он пригодится тем, кто хочет заменить данные, которые были указаны при генерации ключа: электронную почту, имя пользователя или комментарий.

Для изменения привязанных данных нам понадобится идентификатор GPG-ключа — B32440B423383E93. Введём команду в Git Bash — gpg --edit-key B32440B423383E93.

Далее введём adduid для добавления сведений об идентификаторе пользователя и нажмём клавишу Enter. Это ключевое слово расшифровывается как add user id.

Укажем имя пользователя. В нашем случае это будет снова githtmlacademy. После нажмём Enter.
Далее нужно ввести электронную почту и снова нажать Enter.
В комментарии на этот раз укажем GitHub key и нажмём Enter.
Затем проверим, всё ли правильно ввели. Если да, введём латинскую заглавную букву O и нажмём клавишу Enter.
Теперь нам нужно согласиться на замену данных, для этого введём save.
